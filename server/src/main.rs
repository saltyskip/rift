...
/// credentials). `ALLOWED_ORIGINS` is a comma-separated env var; sensible
/// defaults cover prod (`https://riftl.ink`) and local dev. Existing API-key
/// callers from arbitrary origins are unaffected because bearer-token requests
/// don't need `credentials: true`.
fn build_cors_layer(cfg: &Config) -> CorsLayer {
    ...
    if origins.is_empty() {
        // Defaults: marketing site + common local dev ports.
        for s in [
