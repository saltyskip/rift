...
- **`pub` data types live in sibling `models.rs` files, never inline in implementation files.** Implementation files (`routes.rs`, `service.rs`, `middleware.rs`, `repo.rs`, `parsers.rs`, `scope.rs`, `quota.rs`, `dispatcher.rs`) hold logic; `pub` data types — request/response DTOs, query-param decoders, database documents, error enums, axum extension newtypes, anything other modules can import — go in `models.rs` next to the implementation that owns them. The codebase is largely AI-generated; this rule is intentionally **strict** (every `pub` data type, no carve-outs, no "this one is transport-only" judgment calls) so consistency holds across many small contributions. The only exception is **truly private types** (no `pub`, used only within a single file as implementation detail) — those stay inline. Implementation containers — types whose purpose is to host methods or trait impls — stay in their implementation file and are recognized by suffix: `*Service`, `*Repo`, `*RepoMongo`, `*Parser`, `*Dispatcher`, `*Checker`. If your type is data but its name happens to match one of these suffixes, rename it; the suffix should accurately describe role. When extracting, update OpenAPI `components(schemas(...))` registrations in `api/mod.rs` to point at the new path. **Enforced by `src/architecture_tests.rs::pub_data_types_live_in_models_rs`** — the test scans the listed implementation file types at `cargo test` time and fails the build with a list of violations.
