...
- **`pub` data types live in sibling `models.rs` files, never inline in implementation files.** Implementation files (every `.rs` under `src/` except `models.rs`, `lib.rs`, `main.rs`, `architecture_tests.rs`, and `*_tests.rs`) hold logic; `pub` data types — request/response DTOs, query-param decoders, database documents, error enums, axum extension newtypes, anything other modules can import — go in `models.rs` next to the implementation that owns them. The codebase is largely AI-generated; this rule is intentionally **strict** (every `pub` data type, no carve-outs, no "this one is transport-only" judgment calls) so consistency holds across many small contributions. **`pub(crate)` types are exempt** — the rule is about the published external API surface, not crate-internal scaffolding. **Truly private types** (no `pub`, used only within a single file as implementation detail) also stay inline. **Implementation containers** — types whose purpose is to host their own `impl` block (services, repos, parsers, dispatchers, etc.) — stay in their implementation file and are exempted by an explicit `crate::impl_container!(Name);` marker placed above the type definition (see `src/lib.rs` for the macro). The marker is grep-able, decoupled from naming convention, and validated by the compiler — typos fail to build. When extracting data types out of an implementation file, update OpenAPI `components(schemas(...))` registrations in `api/mod.rs` to point at the new path. **Enforced by `src/architecture_tests.rs::pub_data_types_live_in_models_rs`** — the test scans every `.rs` file in `src/` (except the carve-outs above) at `cargo test` time. A small `PUB_TYPES_CLEANUP_BACKLOG` tracks pre-existing violators with `// TODO` comments; new files inherit enforcement automatically.
- **Stepdown rule enforced in tests too.** `src/architecture_tests.rs::stepdown_rule_at_depth_zero` flags any free private `fn` that appears before the first free public (or `pub(crate)`) `fn` at depth 0 in the same file. Methods inside `impl` blocks aren't checked. The caller-above-callee half of the rule isn't mechanically checked but is still review-enforced (the bullet below).
- Three or more `if let` statements in a row applying parallel logic is a smell — there's almost always a flatter form: a struct literal (when each branch sets one field of the same struct), a fluent builder with `Option<T>` setters (when callers need to propagate optionality and you'd value validation/encapsulation/required-field enforcement at the constructor — see `CreateLinkInput`), `.map()` / `.and_then()` (when transforming `Option<A>` → `Option<B>`), `filter_map` (when conditionally building a `Vec`), or `match` (when conditions overlap). `if let` is for *one* conditional unwrap with a side effect, not a substitute for declarative construction. **Builder design note:** if you write a builder, make the setters take `Option<T>` (not `T`) — taking `T` forces every caller into an `if let` chain and reintroduces the smell at every call site.
...
- **File ordering follows Robert C. Martin's stepdown rule (Clean Code).** Source files read top-down: `use` declarations → public API (handlers, service methods — the file's interface) → private helpers at the bottom. Within the public API, when one function calls another, the caller appears above the callee where possible — descend one level of abstraction per function as the reader scrolls down. **Don't bury route handlers under hundreds of lines of helpers.** A reader opening `routes.rs` should see the route handlers first (the file's reason to exist), then helpers in a `// ── Helpers ──` section at the bottom. Exception: a single helper used by exactly one handler and tightly coupled to it (e.g. a multi-step resolver helper) can sit immediately below that handler — locality wins when it's a 1:1 relationship. Otherwise helpers go at the bottom. Same shape applies to `service.rs`: public `impl Service { ... }` methods first, private free functions and validators at the bottom.
