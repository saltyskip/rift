...
- **`pub` data types live in sibling `models.rs` files, never inline in implementation files.** Implementation files (`routes.rs`, `service.rs`, `middleware.rs`, `repo.rs`) hold logic; `pub` data types — request/response DTOs, query-param decoders, database documents, error enums, axum extension newtypes, anything other modules can import — go in `models.rs` next to the implementation that owns them. The codebase is largely AI-generated; this rule is intentionally **strict** (every `pub` data type, no carve-outs, no "this one is transport-only" judgment calls) so consistency holds across many small contributions. The only exception is **truly private types** (no `pub`, used only within a single file as implementation detail) — those stay inline. Service/repo *implementation containers* (`pub struct LinksService { ... }`, `pub struct LinksRepo { ... }`) are not data types and stay in their implementation file. When extracting, update OpenAPI `components(schemas(...))` registrations in `api/mod.rs` to point at the new path. **Enforced by `src/architecture_tests.rs::pub_data_types_live_in_models_rs`** — the test scans `routes.rs` and `middleware.rs` files at `cargo test` time and fails the build with a list of violations. The scope expands in subsequent cleanup PRs as service errors and repo doc structs migrate.
